...
func (ctrl *SetProductController) DeleteSetProduct(c *gin.Context) {
	var SetProduct models.DeleteSetProduct
	if err := c.ShouldBindJSON(&SetProduct); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"status": "400", "message": err.Error()})
		return
	}

	deleteProduct := models.DeleteSetProduct{
		ID: SetProduct.ID,
	}

	if err := ctrl.DB.Where("id = ?", deleteProduct.ID).Delete(&models.SetProduct{}).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"status": "404", "message": "Set product not found"})
			return
		}
